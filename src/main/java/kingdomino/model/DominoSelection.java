...
public class DominoSelection
{
  ...
  public DominoSelection(Player aPlayer, Domino aDomino, Draft aDraft)
  {
    ...
    if (!didAddPlayer)
    {
      throw new RuntimeException("Unable to create dominoSelection due to player. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddDomino = setDomino(aDomino);
    if (!didAddDomino)
    {
      throw new RuntimeException("Unable to create dominoSelection due to domino. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddDraft = setDraft(aDraft);
    if (!didAddDraft)
    {
      throw new RuntimeException("Unable to create selection due to draft. See http://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }
